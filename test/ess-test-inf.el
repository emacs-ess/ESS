...
(ert-deftest ess-path-update-local-portion ()
  (let ((current-local-path "/path/to/file")
    ...
    (let* ((replace-path (lambda (path)
                           (replace-regexp-in-string "path/to/file$" new-local-path path)))
           (check-replacement (lambda (current-path)
                          (string= (ess-path-update-local-portion current-local-path new-local-path)
                                   (funcall replace-path current-path)))))
      (should (funcall check-replacement current-local-path))
      (should (string= (ess-path-update-local-portion current-remote-path-basic new-local-path)
                       "/ssh:melancholia.danann.net:/home/username/projects")))))
