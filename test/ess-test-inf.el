...
(ert-deftest ess-path-update-local-portion ()
  (let ((current-local-path "/path/to/file")
        (current-remote-path-basic "/ssh:melancholia.danann.net:/path/to/file")
        (current-remote-path-ip4 "/ssh:10.0.1.213:/path/to/file")
        (current-remote-path-ip6 "/ssh:[::1]:/path/to/file")
        (current-remote-path-user "/scp:user@host:/path/to/file")
        (current-remote-path-port "/ssh:daniel@melancholia#42:/path/to/file")
        (current-remote-path-domain "/smb:daniel%BIZARRE@melancholia:/path/to/file")
        (current-remote-path-hop "/ssh:bird@bastion|ssh:news.my.domain:/path/to/file")
        (new-local-path "/home/username/projects"))
    (let ((check-replacement
           (lambda (test-path)
             (string= (ess-path-update-local-portion test-path new-local-path)
                      (replace-regexp-in-string "/path/to/file$" new-local-path test-path)))))
      (should (funcall check-replacement current-local-path))
      (should (funcall check-replacement current-remote-path-basic))
      (should (funcall check-replacement current-remote-path-ip4))
      (should (funcall check-replacement current-remote-path-ip6))
      (should (funcall check-replacement current-remote-path-user))
      (should (funcall check-replacement current-remote-path-port))
      (should (funcall check-replacement current-remote-path-domain))
      (should (funcall check-replacement current-remote-path-hop)))))
