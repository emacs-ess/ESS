...
%%% AJR: The next two seem a bit silly?  or need to be rewritten?
%%% 
%\paragraph{\stexttt{iESS} Processing on remote Emacs.}
%Telnet to a remote computer using a telnet client other than an Emacs
%buffer.  Run an Emacs on the remote machine in the telnet window.  Run
%\stexttt{M-x SAS} on the remote machine in that remote instance of
%Emacs.  On a Unix machine, this technique also works for a local Emacs
%and local SAS.

%\paragraph{\stexttt{iESS} Processing on local Emacs.}
%In principle, we can run remote SAS on a local Emacs, but none
%of us has found the need
