...
%\stexttt{.emacs} configuration file:
%\begin{Salltt}
...
input command files, such as \stexttt{myfile.sas}, for SAS.  These are files
...
The \stexttt{*SAS:1.log*} or \stexttt{myfile.log} buffer in \stexttt{ESStr} mode
corresponds to the file \stexttt{myfile.log} in SAS batch usage and to the
...
The \stexttt{*SAS:1.lst*} or \stexttt{myfile.lst} buffer in \stexttt{ESSlst} mode
corresponds to the file \stexttt{myfile.lst} in SAS batch usage and to the
...
Files are edited in the myfile.sas buffer.  The \stexttt{C-c C-r} key in
...
Emacs initialization file (\stexttt{.emacs} or \stexttt{\US emacs}), pointing
...
Examples using the philosophy that S objects are available in the documentatio
...
or 
  \elcode{M-x R}
A new buffer \stexttt{*S+x:1*} or \stexttt{*R:1*} will appear.
...
%\noindent
%Start up \Splus~3.x, in a process buffer (this will be \stexttt{*S+3:1*}) 
%  \elcode{M-x S+3}
...
%  \elcode{C-c C-b}
%Go to the \stexttt{*S+3:1*} buffer, which is the process buffer, and examine
%the results.
%  \elcode{C-c C-y}
...
%We recommend the following line appear in your \stexttt{autoexec.sas}
...
%% Here is a typical \stexttt{myfile.sas}:
%% \begin{Salltt}
...
%the default settings of the \stexttt{inferior-SAS-args} in \stexttt{essd-sas.el}
...
%\stexttt{inferior-SAS-args} and in the \stexttt{autoexec.sas}.  The
...
Revisions to the SAS code are made in the Emacs \stexttt{myfile.sas} buffer.
Useful portions of the SAS log and listing windows can be copied and placed
in new Emacs buffers or dropped directly into a document in Emacs or in another
editor.
\end{document}
