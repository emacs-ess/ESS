...
Note that the Stata CLI is depreciated towards use of a new GUI, and
...
the 3-window graphical terminal interface which is being depreciated
towards a GUI version.
...
enter these, call R using a "prefix argument", by \verb+C-u M-x R+ and
when ESS prompts for "Starting Args ? ", enter (for example):
...
machine.  The idea of \verb|S+elsewhere| is that we open a telnet,
rlogin, or ssh connection to another machine, call the buffer
