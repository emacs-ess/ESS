...
development time of complex software systems in the last decade can be
partially attributed to the use of IDEs and other Rapid Application
Development (RAD) tools.  In the field of statistics, programming is
an important skill which can be augmented and enhanced by the right
tools and environment.  This is especially important with the
...
Carlo (MCMC) sampling (such as Gibbs and Metropolis--Hastings).

Statistical software tools are intended for either ``general'' data
analysis or can target specialized statistical analyses, with the
specialized tools sometimes being orders of magnitude more efficient
for production of data analyses.  However, this is balanced by the
inability of the specialized tools to perform a wide range of common
operations which are available in the general package.  Tightly
coupled interoperability between programs is rare; the need to switch
between tools is unfortunately not.  For example, general purpose
tools such as R \citep{ihak:gent:1996} do not perform Bayesian
analyses as easily as focused tools such as BUGS
\citep{SpieThomBest:1999}.  On the other hand, these specialized tools
lack breadth in the scope of analyses and graphics that can be
generated.  Indeed, BUGS is distributed with applications which
simplify the transfer of BUGS output to R or S for processing the
results using the CODA package.  This suggests a possible gain through
the use of multiple computational tools for complex statistical
analyses, but unfortunately, most tools have their own idiosyncratic
user interfaces which negates this gain in effectiveness.
...
which is called from an interpreted language such as R, in which the
objective function to optimize is written.
...
several files of data and source code for a single analysis. ESS, as
part of Emacs, integrates version code control systems, methods for
access to programs and files on remote networks, an interface to
documentation systems such as \LaTeX\ and \textsc{xml}, as well as
functionality for many common tasks.  In addition, Emacs can be
programmed to perform many tasks related to data cleaning, management,
and editing.

Other complex interface conflicts exist.  For example, few statistical
packages have identical command-line interfaces; this is a distinct
problem from that of syntax differences between macro or programming
languages.  Thus, it might be necessary for a statistical analyst to
learn 2 or 3 different sets of keystrokes for editing and executing
code.  At a higher level, it might be necessary for the analyst to
consider dialects of a language, for example under the S family of
languages, there is R, \Splus, and the original S implementation, as
well as different versions of each of these, in order to verify the
veracity of the statistical package.  Another task, code design and
testing, necessitates working in multiple dialects and versions; this
might mean simultaneously accessing programs which run on completely
different sets of machines.  For example, one might want to be
connected to multiple R processes, in order to verify behavior on
different versions of the same software; perform test-and-run
scenarios where one process is doing long-term processing while the
other is doing short-term testing; simulations; and run multiple
processes on different remote machines from the same location.

ESS mitigates some of the problems noted above.  It provides a well
designed and documented programmer's editor, an interface to
...
The next section discusses statistical user interfaces.  This is
followed by a discussion of ESS and Emacs.  Sections \ref{sec:S} and
...
%% S-Plus 4 for Windows uses MDI because there is a single S-Plus frame
%% in which the commands window, the graphsheets, the objects browser
%% all appear as subwindows. R uses SDI because the Rgui is one frame,
%% the graphsheet is another, and they can be independently moved on
%% the screen.}

%% AJR:  yes to the former.  I thought RGui was the former, not the
%% latter, but I will have to verify.
