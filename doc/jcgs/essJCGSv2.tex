...
%%S\date{\today}
\date{$ $Date: 2002/02/12 18:03:19 $ - $Revision: 1.166 $ $\tiny printed \today}
...
which are nearly identical between file types.
...
remote machine exactly as if they were on the local machine.  The
local Emacs can also monitor and control remote processes running in a
shell buffer.  \stexttt{ange-ftp} and \stexttt{EFS} are used for file
transfers using the insecure ftp protocol.  \stexttt{tramp} is used
for file transfers using the secure shell protocol.
...
only differs from interactive processing by the number of commands
being evaluated.  The second is the load-source mechanism, which
provides a means of jumping to errors in the source file.  Both of
these mechanisms provide tools for debugging the source files.  Once
the files are debugged, S languages have a standard feature for batch
evaluation, which is independent of ESS.
%feature
%batch file is command errorscope of the is
