...
%%S\date{\today}
\date{$ $Date: 2002/02/15 01:05:20 $ - $Revision: 1.173 $ $\tiny printed \today}
...
\begin{abstract}
  ...
  an intelligent user interface to facilitate this.  ESS runs under
  the Unix, Microsoft Windows, and Macintosh operating systems and
  interfaces with SAS, S-PLUS, R, and other statistics packages.  ESS
  is based on the Emacs text editor and uses its features and
  extensions to streamline the creation and use of statistical
  software applications.  In particular, ESS provides features for
  editing source files and programs for statistical programming and
  data analysis, as well as assist with interacting with particular
  statistics packages without leaving Emacs.  We discuss how ESS works
  and how it can increase statistical programming efficiency.
\end{abstract}
