...
%%S\date{\today}
\date{$ $Date: 2002/02/19 04:44:58 $ - $Revision: 1.188 $ $\tiny printed \today}
...
and showing how it facilitates statistical computing.
%  The unique
We give a short history of the development of ESS.
% as an open-source project follows; it is a
% synthesis of several independent projects and has passed through 
% generations of developers.
We conclude with future extensions and related work.
...
% too much information -- based on some function of time
% and the amount of editing activity.  
...
% (or by typing the command name itself).
%% rmh: The user presses a key, not a map.
%%rmh: don't need this:
% Not all functions are interactive, and
% only those that are can be bound to a key sequence.
...
%, perhaps with a small amount of customization, for 
...
cursor.  Minor modes can emulate the
...
%processors is the notion of flexible source code versioning.  
...
% The actual data analysis commands entered can be
...
% Rodney:  these are Emacs features.  Why are they in the ESS Extends
% Emacs section?
...
% Rodney:  same comment as above.
%Other features which provide potential for efficient
...
ESS (Emacs Speaks Statistics) is built on Emacs, the editing system for
...
\Fortran\ mode made available in 1986.  As statisticians' preferences changed
from generalist compiled languages such as \Fortran\ to specialist interactive 
statistical analysis packages like S and SAS, Emacs modes for S and SAS soon 
followed.

Doug Bates and Ed Kademan wrote S-mode in 1989
to edit S and \Splus\ files in GNU Emacs.
Frank Ritter and Mike Meyer added features, creating  version 2.
Meyer and David
Smith made further contributions, creating version 3. David Smith
provided process interaction changes for version 4.
...
Tom Cook wrote SAS-mode in 1994, partially basing it on GNU Emacs
macros written by John Sall.  Rossini extended SAS-mode to work with
XEmacs in 1995.

In 1995 Rossini merged S-mode and SAS-mode into a single Emacs package
for statistical programming.  The product of this marriage, along with
additional changes to accommodate multiple languages in a flexible
way, was called ESS version 5.
...
use the standard-input/standard-output model of inter-process
...
Exchange (DDE) protocol.  Heiberger used DDE to provide
interactive interfaces for Windows versions of \Splus.
