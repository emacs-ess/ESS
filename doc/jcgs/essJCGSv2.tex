...
%%S\date{\today}
\date{$ $Date: 2002/02/04 20:21:32 $ - $Revision: 1.98 $ $\tiny printed \today}
...
%  localize the probable location of the error.  With \SAS\ log files,
  %we 
  automatically placing the cursor on the first \stexttt{ERROR}, for example in \SAS and S. %in =SAS and 
...
\SAS\ listing buffers.  Note that %% When accessed in this manner, 
...
of developers, with 5 total projects.  ESS descended from and combined
2 Emacs modes: S-mode and SAS-mode.

\begin{figure}[tbp]
  \centering
  \includegraphics[height=3in,width=5.5in]{timeline.eps}
