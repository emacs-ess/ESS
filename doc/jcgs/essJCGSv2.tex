...
%
\else%% Make all the comments invisible by using the following instead
%% >>>>>>>>> NOT YET --- WHO can do this? {MM: I'll find out sooner or later}<<
\newenvironment{Comment}{\renewcommand{\baselinestretch}{}%
  \begin{quote}\tiny\itshape }{\end{quote}\renewcommand{\baselinestretch}{1.5}}
%%--- how does one do this?
%% 1) only eats one item :
%% \newcommand{\Comment}[1]{}
%%
%% 2) alsoonly eats one item :
%%- \newcommand{\eatMYarg}[1]{}
%%- \newenvironment{Comment}{\tiny\begin{EatALL}}{\end{EatALL}}
%%- \def\EatALL{\par\eatMYarg}
%%- \def\endEatALL{}
\fi
...
%%S\date{\today}
\date{$ $Date: 2002/01/07 23:01:36 $ - $Revision: 1.39 $ $\tiny printed \today}
...
them are hard core Unix fanatics.  However, I see no problem with changing, 
since it is only loosely based on ESS[SAS].  Luckily, I only have to change 
one key-binding since there aren't any others.  What is the R/S key-binding 
for batch submit?  I'll remove this comment after I change ESS[BUGS] (and 
somebody tells me what to change it to :0)  Thanks.
\end{description}
\end{Comment}
