...
%%S\date{\today}
\date{$ $Date: 2002/02/21 21:21:21 $ - $Revision: 1.240 $ $\tiny printed \today}
...
\citep{ihak:gent:1996,R}; \SAS\regstrd\ \citep{SAS:8}; \XLispStat\ 
...
understand their filetype's syntax and grammar and therefore provide
...
Syntax highlighting can be used to help enforce company coding
...
DDE server.  ESS can control the actions of the package but can not capture
...
output protocol nor DDE can not be directly controlled by Emacs.  ESS
provides an editing environment for their statistical languages.  The
...
interface to the graphical routines and the help system.  Data analysis
...
only exist on the computer running \SAS.  The net result is that
