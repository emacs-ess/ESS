...
%%S\date{\today}
\date{$ $Date: 2002/02/05 22:39:25 $ - $Revision: 1.107 $ $\tiny printed \today}
...
components have merged over time.%.; ESS is
%unique as an open-source, free-software project that has been passed
%down by multiple generations of developers and has united various
%Emacs extensions which support statistical language programming.
...
is freely available under the GNU General Public License (GPL) for a large
...
implementations of Emacs which are released for use under the GPL:  GNU 
Emacs \citep{GNU-Emacs} and XEmacs
...
%, including intelligent completion as well as content presentation.
%% When opening a file, Emacs can complete partially-typed file names by
%% referring to the operating system file directory.  
...
single display.  Presently, Emacs is more often used with a GUI,
%in a windowing environment within its own window, 
but binding commands to keystrokes provides an important time and
memory saving feature.  Emacs menus and toolbars on the display screen
allow mouse access to frequently used actions and provide a graphical
alternative when the user does not know or can not recall a keymap.
...
specified intervals.  One common program that is run within Emacs
...
of the interaction can be edited and searched while the program is
still running.
...
only those that are can be keymapped.  
...
colorization; and many other features.
...
%  \emptyfig
%\else
%\psfig{file=ediff-sas.ps.gz,angle=270,width=\textwidth}
%\fi
...
Comparison of files, two or three drafts of a paper for example,
is simplified by \stexttt{ediff}.
...
Emacs \stexttt{dired} mode (discussed in section \ref{sec:modes})
interfaces to the computer's directory structure to provide a
file-manager capability.   Emacs command-history record goes back
to the beginning of an editing session, allowing an infinite undo capability.
Other extensions to Emacs allow it to act as a web browser, a
...
%  \emptyfig
%\else
%  \psfig{file=hilock-sas.ps.gz,angle=270,width=\textwidth}
%\fi
...
local directories is also available.%  In addition (currently only for
%S languages), there exists object-name and function-name completion;
%these are based on tables which can be reconstructed automatically or
%manually when the object database in S changes.
...
useful for exploratory data analysis.% with descriptive statistics and graphics.
