...
%%S\date{\today}
\date{$ $Date: 2002/02/06 22:20:25 $ - $Revision: 1.126 $ $\tiny printed \today}
...
Statistics (ESS) \citep{ESS}, a software package which
...
statistical code and with evaluation of that same
code for the data analysis.  ESS works with software packages
running in either interactive or batch modes.
ESS currently supports
...
Emacs text editor, the platform upon which ESS runs.  Next,
the role of ESS as the statistician's user interface is presented
detailing its operation and showing how it facilitates common
statistical computing activities.  The unique history of ESS as an
open source project follows; it has been passed down
through many generations of developers and has grown as a synthesis
of several previously independent components.
...
%% which are derived from a  (technically, both are special cases of
%% \stexttt{ess-mode}). 
