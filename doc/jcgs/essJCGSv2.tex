...
%%S\date{\today}
\date{$ $Date: 2002/02/15 21:23:29 $ - $Revision: 1.178 $ $\tiny printed \today}
...
their key-map.  Not all functions are interactive, and
only those that are can have key-bindings.  The most important
extensions to Emacs are called modes.  Modes are of two kinds, 
either major or minor,
depending on the extent to which they take over the editing behavior.

Major modes provide a customized environment consisting of key-maps
...
ftp) and secure (\stexttt{tramp} uses scp or ssh) are
available. %The local 
...
There are many other important features which Emacs has.
% which, while innovative when introduced, are fairly common now.  
Emacs provides file-manager
capabilities such as \stexttt{dired} (discussed in Major and Minor
Modes above), which interfaces to the computer's directory
structure.  Emacs stores the history of commands issued in
an editing session so they can be undone if necessary.  More
importantly, for modes which control processes, the process input
history is stored.  Therefore, the actual commands entered can 
be edited for printing or re-use.
...
and spell checking.
% Rodney: You can't just whip out complex functionality like etags and 
% speedbar.  We would need to introduce these packages.  Since they 
% aren't critical to ESS at this time, let's ignore them.

Emacs is an extremely powerful editor.  
%Emacs has been claimed to
...
computer usage, include the use of source-code control
...
elisp which can be mapped to a key-map.
...
ESS facilitates the editing of source code files which are sets of 
commands written for a statistical analysis package.  This allows the
user to load and error-check small sections of source
code.  This is done through several mechanisms.  First, the presence of
unbalanced parentheses and mismatched/unterminated quotes is immediately 
