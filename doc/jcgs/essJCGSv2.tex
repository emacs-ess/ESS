...
%%S\date{\today}
\date{$ $Date: 2002/02/21 21:41:52 $ - $Revision: 1.241 $ $\tiny printed \today}
...
\citep{Stata:7.0}; \XLispStat\ \citep{Tier90} and its extensions 
Arc \citep{Cook:Weisberg:1999} and ViSta \citep{youn:fald:mcfa:1992}; 
BUGS \citep{BUGS}; and Omegahat \citep{DTLang:2000}.  ESS can be extended to
...
Emacs capabilities are extended by loading text 
%or byte-compiled 
...
file editing are often determined by the file type or extension, i.e.
the characters at the end of the file name that follow a period like
\stexttt{txt}, \stexttt{s} or \stexttt{sas}.  Examples of this kind of
major mode are \stexttt{ESS[S]} and \stexttt{ESS[SAS]}.  Major modes
understand a file type's syntax and grammar and therefore provide
...
which provides features commonly found in such environments that have
been customized for statistical languages.  It provides a single
...
package redirected to the buffer.

Packages that do not use the command-line interface must be run as an
independent process, possibly with limited cooperation.
...
on a remote machine with \stexttt{iESS[S]} or \stexttt{iESS[SAS]} (see below).
%, using the command \stexttt{S+elsewhere}.  
All the editing and interaction features described for the local machine
work equally well on the remote machine.  The interaction, including
all the unique features of working with ESS, appear to the user
as if the program were running on the local machine.
...
Unix systems onto a local Microsoft Windows or Apple Mac display.
...
the full editing capabilities of Emacs.  Second, \Splus\ language commands
are sent from the editing mode \stexttt{ESS[S]} buffer and from
transcript buffers from previous \Splus\ sessions directly to the GUI
...
interface, with Emacs key sequences.
...
%can fix it later.
%
%The \SAS\ Display Manager provides a form of interaction between the
%user and the \SAS\ program, particularly with the mouse-based
%interface to the graphical routines, the help system.  Data analysis
%code can be edited in \stexttt{ESS[SAS]} mode in Emacs and then
%forwarded to \SAS\ for processing.
%
...
%Rodney:  It seems to be a funny way to introduce batch processing.
%
%Exploratory data analysis is most often accomplished interactively.
%Situations exist for which interactive analyses are problematic.
...
statistical reports and figures.
...
ESS facilitates BUGS batch with \stexttt{ESS[BUGS]}, the mode for files
with the \stexttt{bug} extension.  ESS provides 4 features.  First, 
BUGS syntax is described to allow for proper fontification of statements,
...
4.6.  In 1995, Rossini extended SAS-mode to work with XEmacs.  And,
S-mode version 4.7 was available for both GNU Emacs and XEmacs and supported
S, \Splus, and R.
