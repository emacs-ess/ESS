...
found in \cite{heiberger:dsc:2001}.  Examples of its use are included in the
documentation that comes with the ESS source code.  Details of the implementation
of ESS are also provided.  We first describe the Emacs editor, the platform on which ESS
...
Emacs can be used in a terminal, keyboard-only, implementation
on those platforms which support it.  But, most platforms also
provide a graphical user interface (GUI) allowing the user to interact
with either the keyboard or a mouse.  Therefore, recent versions of Emacs
provide GUI support.  Emacs menus and toolbars allow mouse access to frequently used actions
for instances when you don't know or can't remember the keyboard
...
Emacs capabilities are extended by packages written in a
dialect of Lisp called Emacs Lisp which is also known as E-Lisp 
\citep{RChassell1999,PGraham:1996}.  The main E-Lisp
construct is called a function and related functions are grouped together into
packages.  Packages may provide their support in the form of what are called modes.  Major-modes
provide support for types of files determined by the file name;
specifically the file name's extension which are the characters at the end
