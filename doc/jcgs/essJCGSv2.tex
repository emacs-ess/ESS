...
%%S\date{\today}
\date{$ $Date: 2002/02/06 00:42:11 $ - $Revision: 1.117 $ $\tiny printed \today}
...
\paragraph{Buffers give Emacs control}%for Editing and for Program Control.}
...
still running.  %% We talk about how this affects other things\ldots
...
Major-modes provide a %two kinds of support.  First, they provide a
...
\stexttt{ESS[SAS]}.% which are derived from a  (technically, both are special cases of
%\stexttt{ess-mode}).
...
%% No need to bring up all the details, which will become obsolete. 
...
file-manager capability.  Emacs command-history record goes back to
the beginning of an editing session, allowing a near-infinite undo
capability.  Other extensions to Emacs allow it to act as a web
browser, a mail and news reader, and as an interface to other common
text-based tools such as spell checking programs.
