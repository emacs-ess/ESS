...
%2.  what emacs has to offer statisticians
%        a) buffers
%        b) key-binding
%        c) other packages
%                1) font-lock
...
%3. what ESS has to offer statisticians
%        0) font-lock/key-binding/other packages revisited
%        -- incorporated in new 3.1 and 3.2 below
%        1) interactive
%           a) R/S
%           b) SAS
%        2) batch
...
%%S\date{\today}
\date{$ $Date: 2002/01/12 22:14:35 $ - $Revision: 1.51 $ $\tiny printed \today}
...
\begin{verbatim}
c:/emacs/ess-washington/ess/doc/jcgs\$ latex \\input essJCGSv2.tex
...
ESS supplies function key-mappings as defined by \SAS\ Display Manager.
These keys are defined, by default, only when editing files with the 
\stexttt{sas} extension.  Optionally, they are available in all modes which
is very useful when you are editing \SAS\ log and \SAS\ output with 
extensions of \stexttt{log} and \stexttt{lst} respectively.

With one key-map, you submit a \SAS\ batch process.  With others, you 
move between the \SAS\ program, the \SAS\ log and the \SAS\ output.  When 
accessed in this manner, the \SAS\ log and \SAS\ output are automatically 
updated to the latest as they are being appended by the \SAS\ batch process.  
In addition, the \SAS\ log is searched for error messages and the error 
messages, if any, are sequentially displayed with consecutive key presses.  
...
key-map to move the cursor to the left to a previous tab-stop deleting any 
...
Lastly, ESS cooperates with remote file access modes \stexttt{ange-ftp},
\stexttt{EFS} and \stexttt{tramp} by remotely submitting \SAS\ batch processes
to the remote computers that the files reside on.  This is important since
any \SAS\ permanent datasets referenced in the \SAS\ programs only exist
on those remote computers.  Kermit file transfers are also supported.  The
remote editing and statistical processing of files is called ESS-elsewhere and 
...
extensions written by Martin M{\"a}chler, were incorporated to what became
...
for R.
%%, in particular, providing Rd mode.
...
%need to move this discussion to interactive SAS above.
...
Most of the work, until 1998, was for Unix statistics packages that used 
either standard-input/standard-output or batch file processing for 
inter-process communication.
% for both the command-line interface and batch file
%%processing.  
ESS could not communicate with Windows versions of statistical packages 
that did not offer these methods.
%use the standard-input/standard-output model.  
This hurdle fell in 1998 when Brian Ripley designed a way for ESS to use
the Windows technology of dynamic data exchange (DDE).  Based on this work, 
Heiberger provided interactive interfaces for Windows versions of \Splus.
...
%moved this discussion to a new section above
...
ESS is software that is constantly in flux.  New versions of statistical
...
http://software.biostat.washington.edu/statsoft/ess/.  To get help
with problems you can send e-mail to ess-help@stat.math.ethz.ch.  The latest
unstable version can be obtained from anonymous CVS.  First type:

\stexttt{cvs -d :pserver:anoncvs@software.biostat.washington.edu:/home/ess/repository login}

Enter the password ``anoncvs''.  Then type: 
