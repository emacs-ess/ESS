...
%%S\date{\today}
\date{$ $Date: 2002/01/03 14:50:35 $ - $Revision: 1.33 $ $\tiny printed \today}
...
%% ESS has customizable automatic indentation, with the customization based
...
\begin{quote}
Rodney:  we need a blurb in the text about what Rd-mode is
\end{quote}
...
of the log.  This provides a powerful development environment for \SAS.  
%%The SAS-mode batch file processing
%%works identically whether the \SAS\ process is running on the same machine
%%or on a remote machine.

\begin{quote}
Rich:  Actually inferior SAS for interactive access to remote SAS, as
described above, was available before ESS-elsewhere was designed.
The SAS batch file processing that you and I designed works equally
well on local or remote SAS systems.   I think the modifications I made to
the description of inferior SAS and batch SAS are sufficient.

Rodney:  I just checked and remote SAS batch processing could not
possibly work since the directory was part of the command, i.e.
sas /path/file  So, I changed it to sas file  But, it still
will not work since the cd command has to distinguish between host
and path, i.e. [host]/path for tramp or /host:/path for ange-ftp and EFS.
I'll try to get this working, but until then; the comment stands.
\end{quote}
