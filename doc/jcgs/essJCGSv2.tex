...
\ifx\pdfoutput\undefined
  %% Stuff wout hyperref
  \def\url#1{\stextsf{#1}} % To help fit in lines
\else
  %% Stuff with hyperref
  \usepackage{hyperref}
...
%%S\date{\today}
\date{$ $Date: 2002/01/13 07:13:08 $ - $Revision: 1.52 $ $\tiny printed \today}
...
``clean'' transcripts from S languages by finding all input lines and
isolating them into an S language source file.  Transcript cleaning
simplifies the construction of an idealized source file, perhaps to be
used routinely in batch analyses of similar data sets, from an
exploratory interactive analysis session.
...
\begin{Comment}
Rich: Rodney, the standard input/standard output model is a single model.
It is used for interaction when stdin/stdout refer to the terminal.
It is used for batch when they refer to the card reader and the line printer.
There is no difference in the program.  I restored my wording.
\end{Comment}
