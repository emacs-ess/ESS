...
Emacs is a mature, powerful, and extensible text
...
Unix, Microsoft Windows and Apple Macintosh.  There are two versions,
or forks, of Emacs (GNU Emacs \citep{RMS:2000} and XEmacs).  Emacs shares many
...
%as well as a few commercially support versions (UniPress Emacs).  All
%share extensibility through Lisp programming.  Versions have been
%written which use Python and PERL as extension languages, but these
%are in much less common use.
%Emacs was originally written for the first screen-mapped terminal
%devices, and this is still manifested in its use of keymappings, which
%have been stable for many, many years.
Emacs was originally written when terminals were the only method of access
to computers.  Emacs commands were assigned to key sequences which is 
known as key-mapping or key-binding.  Currently, using Emacs as if it
were running on a terminal, is only done in special circumstances like
a few quick edits or remote access to a computer via a modem or the 
Internet.  Over the last decade, Emacs has
been extended to use windowing systems such as X11, Microsoft Windows,
and Apple Macintosh.  These windowing systems provide a graphical user
interface (GUI) which allows the user to interact with either a keyboard or
a mouse and provides applications with the ability to share space on a
single display.  Emacs menus and toolbars allow mouse access to
frequently used actions; for instance, when you don't know or can't
remember the appropriate key-mapping.  User-defined menus and toolbars can
be constructed as needed.

However, Emacs provides facilities which go beyond ordinary text
editing.  Emacs creates a dedicated space on the display, called a
buffer, to perform tasks such as editing a text file on disk or
controlling other programs.  You can switch between buffers to observe
or make progress on concurrent tasks.  Buffers allow the user to edit
multiple files and control multiple programs simultaneously.
...
Emacs's power and flexibility make it a reasonable starting choice to
