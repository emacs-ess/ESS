...
%%S\date{\today}
\date{$ $Date: 2002/02/19 20:22:30 $ - $Revision: 1.205 $ $\tiny printed \today}
...
%and also has the ability to initial secure socket connections using SSL.
%rmh: something wrong with the ssl sentence.  also I don't think we need it.
...
protocol.  This changed in 1998.  Brian Ripley demonstrated how ESS 
could use the Windows Dynamic Data Exchange (DDE) protocol.  
Heiberger used DDE to provide interactive interfaces for Windows 
versions of \Splus.  Rodney Sparapani designed an ESS batch file 
processing system for \SAS.  The system relies on files, 
rather than standard-input/standard-output, and works
