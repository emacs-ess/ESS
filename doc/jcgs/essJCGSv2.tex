...
%%S\date{\today}
\date{$ $Date: 2002/02/12 19:24:22 $ - $Revision: 1.170 $ $\tiny printed \today}
...
analysis code for both interactive and batch-file driven statistical
...
This paper starts by describing the Emacs text editor; this forms the
underlying platform for ESS.  Next, we discuss how ESS is involved in
enhancing a statistician's daily activities, by presenting its
features, operation, and how it facilitates statistical computing.
The unique history of ESS as an open source project follows; it has
been passed down through many generations of developers and represents
a synthesis of several independent projects.  We conclude with future
...
which go beyond ordinary text editing.  More importantly to our goals,
...
statistics package is normally captured directly by Emacs, from which
...
run programs which might reside on numerous remote machines during the
...
one-way communication to the \Splus\ GUI process, but can not capture
...
both transparent editing of files and execution of statistics package
