...
activity involves computation.  Research, data analysis, and
communication usually require some form of computer usage.  The user
interface, which acts as a buffer between the statistician and the
computer can help and hinder work.  This paper describes ESS, which
provides a customizable interface to a number of common statistics
packages through the Emacs editor.  This interface facilitates a
common set of tasks for data analysis, code management, and
programming.

The tasks involved in data analysis are the same, or can be considered
to be very similar, to those tasks found in computer programming.
While some are obviously programming, some tasks, such as interactive
data analysis, are similar to visual programming.  

ESS provides a well designed and documented programming editor,
an interface to statistical processes, and provides, through Emacs,
additional tools which aid both statistical software development and
data analysis.  It works with common statistical software, including
the S languages (which include S \citep{BecRCW88,ChaJH92,ChaJ98},
\Splus{} \citep{Splus}, and R \citep{ihak:gent:1996}); \XLispStat\ 
...
found in \cite{heiberger:dsc:2001}.  Details on the programmer API for
ESS as well as examples of its use are included in package
documentation that comes with the ESS source.  We first describe the
basic deployment platform, Emacs.  This is followed by details on ESS
and how it facilitates statistical activities.  ESS's unique history
...
a set of Emacs macros written by John Sall of SAS.  The initial
