...
%%S\date{\today}
\date{$ $Date: 2002/02/07 00:38:29 $ - $Revision: 1.142 $ $\tiny printed \today}
...
%%% AJR: previous, or should we revert to the next line?
%% ESS\@.   
...
remote machine exactly as if they were on the local machine.  Emacs
can also monitor and control remote processes which are running in a
shell running in a local buffer.  \stexttt{ange-ftp} and \stexttt{EFS}
...
These revision control systems facilitate tracking edits and changes
to a file as well as for documenting what was changed during a
particular revision.  More importantly, they allow for branching and
...
%%% AJR: can't reference a paragraph 
%% (discussed in section \ref{sec:modes})
...
stored; this is, for example, the actual data analysis commands
entered and can be very different from observed buffer due to editing.
...
and \SAS\ proc/run combinations, across files within a directory.
...
ESS extends Emacs to provide a functional, uniform, and easily
extensible interface for multiple statistical packages.  This is done
by first, adding shortcuts and features for accelerated editing of
files, and second, by interacting with the statistical language
through a number of activities: general input/output handling,
specialized parsing of help files, and evaluation assistance.
...
output of the package goes directly to an editable buffer whose major
mode is crafted to allow for command-line editing.  This mode
...
\begin{table}[tbp]
  ...
  \begin{tabular}{cccccc}
    ...
    2002 & v.5.2 & \\
    %% Should we send this table around to the previous generations?
    %% Anything else  
    %% they should see?  Of course, we should have an Acknowledgement
    %% section at the  
    %% end.
  \end{tabular}
