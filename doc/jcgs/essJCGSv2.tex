...
%%S\date{\today}
\date{$ $Date: 2002/01/12 19:39:20 $ - $Revision: 1.50 $ $\tiny printed \today}
...
Emacs provides the editing environment which is enhanced by ESS.
...
These keys are defined by default only in \stexttt{ess-mode}, but an option
is provided so that they are available in all modes which can be helfpul
when you are editing files with extensions like \stexttt{txt} or \stexttt{dat}.
With one key-map, you can submit a \SAS\ batch process.  With others, you 
can move between the \SAS\ program, the \SAS\ log and the \SAS\ output.  When 
accessed in this manner, the \SAS\ log and \SAS\ output are automatically 
updated to the latest as they are being appended to by the \SAS\ batch process.  
In addition, the \SAS\ log is searched for error messages and the error messages, 
if any, are sequenced through with consecutive key presses.  
...
destructive previous tab or back-tab). 
...
to the same remote computers that the files reside on.  This is important since
the \SAS\ permanent datasets referenced in those files are only likely to exist
on the same remote computers.  Kermit file transfers are also supported.  The
...
in a typical analysis is quite large and can be very time-consuming.  Most BUGS 
