...
%%S\date{\today}
\date{$ $Date: 2002/02/15 23:15:38 $ - $Revision: 1.181 $ $\tiny printed \today}
...
mapped (or bound) to key sequences for convenience.
% which are called key-maps (or key-bindings).  
Over the
...
elisp which can be mapped to a key.
...
% are less commonly employed by statisticians but 
...
might be used.  That is, the same key-maps are used for cursor movement, 
...
% what the authors prefer is certainly not germaine.
...
Finally, key-mappings are defined to create a command file and submit 
