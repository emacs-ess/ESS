...
%%
  ...
  Thanks go to the referrees and AE for suggesting changes which
  substantially improved the presentation of the paper.}}

\date{\today}
%%\date{$ $Date: 2002/09/26 23:05:10 $ - $Revision: 1.12 $ $\tiny printed \today}
...
\begin{abstract}
  ...
  research and data analysis, and is necessary to use sophisticated
  statistical packages and to write custom scripts and software to
  perform data analysis using modern statistical methods.  Emacs
  Speaks Statistics (ESS) provides an intelligent and consistent
  interface between the user and software.  ESS interfaces with SAS,
  S-PLUS, R, and other statistics packages under the Unix, Microsoft
  Windows, and Apple Mac operating systems.  ESS extends the Emacs
  text editor and uses its many features to streamline the use and
  creation of statistical software.  ESS understands the syntax for
  numerous data analysis languages and provides consistent display and
  editing features across packages.  ESS assists in the interactive or
  batch execution by the statistics packages of statements written in
  their languages.
  ...
  We discuss how ESS works to increase statistical programming
  efficiency.
\end{abstract}
...
A familiar, coherent, and probably most importantly, well-understood
...
to accommodate most statistical packages which provide either a
interactive command-line prompt for inputing instructions, or process
batch files for instructions.

We start by discuss how ESS enhances a statistician's daily activities
by presenting its features and showing how it facilitates statistical
computing.  Next, we describing the Emacs text editor, the underlying
...
forms will be referred to as programming (the activity) or code (the
textual result) in the sequel.  These programs might be written in a
general compiled computer language such as \Fortran\ or C.
Specialized scripting languages such as S, R, \SAS, \Stata, or
\XLispStat are also a popular option among statisticians; however,
they do not have the same range of sophisticated programming tools
such as integrated development environments (IDEs) for writing and
debugging code.

ESS extends the Emacs text editor to provide an IDE for statistical
languages.  The result is an environment with features targeting the
complementary goals of statistical programming and data analysis.  It
offers a single interface for a variety of statistical computing tasks
including interactive data analysis and statistical programming.  ESS
is able to provide a functional and extensible interface which is
uniform and consistent across multiple statistical packages.  This is
done by adding shortcuts and features for accelerated editing of files
as well as by interacting with the particular statistical packages to
provide, for example, control of input/output, assistance with
evaluation, and specialized parsing of help files.
...
ESS provides many features which enhance the construction of data
analysis scripts and statistical programming. Some do not require
direct interaction with a statistics package; these are textual script
editing features such as syntactic fontification and colorization of
scripts, detection of balanced and unbalanced parens and braces
through highlighting, code indentation for readability, commenting out
regions, and indexing of files by function.  However, the more novel
features, such as help file parsing and viewing, object and function
name completion, evaluation of scripts, and interactive editing of
data and objects, require some interaction with the target statistics
package through either interaction or by batch processing.

\subsection{Editing advantages}
...
\textbf{RODNEY/RICH: CAN SAS BE REFORMATTED?}
...
benefits through the availability of useful editing features.  In this
case, the package's GUI tools are available for use.  Examples of this
last situation include the Microsoft versions of SAS, Stata, and
XLispStat.
...
a local Microsoft Windows or Apple Mac display.
...
them all available in the same, extensible, editor.
...
general-purpose compiled languages such as \Fortran\ to specialized
