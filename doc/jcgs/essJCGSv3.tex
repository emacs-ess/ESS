...
\date{\today}
%%\date{$ $Date: 2002/09/27 19:12:57 $ - $Revision: 1.15 $ $\tiny printed \today}
...
\begin{abstract}
  ...
  research and data analysis, and is a necessary for using
  ...
  execution by the statistics packages of statements written in their
  languages.
...
statistical computing.  Next, we describing the Emacs text editor, the
...
forms will be referred to as programs or code (the textual result) or
programming (the activity) in the sequel.  These programs can be
written in a general compiled computer language such as \Fortran\, C,
C$++$, or Java. However, specialized scripting languages supporting
...
environment (IDE) for statistical programming languages. In
...
unbalanced parens and braces through highlighting, code indentation
...
interaction with the target statistical package through either
direct interaction or indirectly through batch processing.
