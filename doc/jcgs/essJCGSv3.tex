...
\date{\today}
%%\date{$ $Date: 2002/09/25 22:05:45 $ - $Revision: 1.7 $ $\tiny printed \today}
...
communication, involve some form of computing.  In particular, this is
exemplified in the notion that many daily statistical activities can
be considered to be \textit{programming with data} \cite{ChaJ98}.
Thus the computer user interface's central role is to facilitate
statistical tasks.  While presentation of character and graphical
information is the most visual component of a user interface, perhaps
a more critical component is how the computer interprets user input.
A familiar, well-understood set of input behaviors can increase the
efficiency of statistical practice.  This paper introduces Emacs
Speaks Statistics (ESS) \citep{ESS}, a software package which provides
a common interface to a variety of statistical packages on the most
...
or batch interface.  It was as a reaction to enhancing this interface
that ESS was initially concieved.  The second approach is to provide a
graphical user interface (GUI); this approach either supplements or
completely replaces the command-line with the use of toolbars and
menus along with dialog boxes for performing statistical procedures.
Data display is provided through the use of a spreadsheet for entry
and modification.  ESS provides a middle ground, with a focus on
writing programming code for statistical computing, while providing
tools, sometimes with associated GUIs, to speed up the associated
programming tasks.
...
ESS extends the Emacs text editor to provide an integrated development
environment for statistical languages, to satisify the complementary
goals of statistical programming and data analysis.  It offers a
single interface for a variety of statistical computing tasks
...
output protocol nor DDE can not be directly controlled by Emacs.  But,
...
form of modes, which provide specific enhancements to the editing
behavior.
