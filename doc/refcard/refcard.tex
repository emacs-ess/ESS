...
\begin{center}
  ...
  {\small updated for ESS 12.09}% {\footnotesize --- needs \em{more} updating!}}
  \\[1ex] {\tiny \SVNDate}
...
\begin{tabTit}{Evaluation commands (Prefix \texttt{C-u}: \emph{in/visibly})}
  \texttt{M-C-x}   & \underline{Evaluate region or function or para} \\
  \texttt{C-c C-c} & \underline{Evaluate region or para.\ or function \emph{\& step}}\\
  \texttt{C-\RET} & \underline{Evaluate region or line \emph{\& step}}\\
  \texttt{C-c C-l} & Load this buffer -- detect errors !\\
  \texttt{C-c C-n} & Step through code -- line by line \\
...
\begin{tabTit}{Others}
  ...
  \texttt{C-c C-z} & Return to process (at prompt)
\end{tabTit}
...
\begin{tabI}
  ...
  \texttt{i}   & Display \textbf{a}propos\\
...
\begin{multicols}{2}
  ...
  Basic commands (default prefix \texttt{C-c d}):
  ...
  Use a custom prefix key (a suggestion): 
