...
(defun ess-rterm-arch-version (version)
  "Find an architecture-specific name for VERSION, an absolute path to R on Windows.
\"R-x.y/bin/Rterm.exe\" will return \"R-x.y\", for R-2.11.x and older.
\"R-x.y/bin/i386/Rterm.exe\" will return \"R-x.y/i386\", for R-2.12.x and newer.
\"R-x.y/bin/x64/Rterm.exe\" will return \"R-x.y/x64\", for R-2.12.x and newer."
  (let* ((dir  (directory-file-name (file-name-directory version)))
	 (dir2 (directory-file-name (file-name-directory dir)))
	 (version1up (file-name-nondirectory dir));; one level up
	 (version2up (file-name-nondirectory dir2));; two levels up; don't want "bin" ...
	 (version3up (file-name-nondirectory ;; three levels up; no "bin" for i386, x64 ...
		      (directory-file-name (file-name-directory dir2)))))
    (if (string= version2up "bin")
	(concat version3up "/" version1up) version2up)))
	...
	(while versions
	  (let* ((version (car versions))
	    ...
	    (when ess-microsoft-p
	      (setq version (ess-rterm-arch-version version)
