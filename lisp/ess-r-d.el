...
\"R-x.y/bin/x64/Rterm.exe\" will return \"R-x.y/x64\", for R-2.12.x and newer."
  (let* ((version1up)
	 ...
	 (version3up))
    (setq
     version1up (file-name-nondirectory ;; one level up
		 (directory-file-name (file-name-directory version)))
     version2up (file-name-nondirectory ;; two levels up; don't want "bin" ...
		 (directory-file-name (file-name-directory
				       (directory-file-name (file-name-directory version)))))
     version3up (file-name-nondirectory ;; three levels up; don't want "bin" for i386 and x64 ...
		 (directory-file-name (file-name-directory
				       (directory-file-name (file-name-directory
							     (directory-file-name (file-name-directory version))))))))
    (if (equal version2up "bin")
	...
	(while versions
	  (let* ((version (car versions))
	    ...
	    (when ess-microsoft-p
	      (setq
	       version (ess-rterm-arch-version version)
