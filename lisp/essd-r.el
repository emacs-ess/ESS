...
;; SJE: 2007-07-11 -- Perhaps this implementation should use an alist
;; rather than two lists, one for version names, and one for version times?
;; e.g. '( ("R-1.9" . 2000) ("R-2.0" . 3000) ("R-devel" . 5000))

;; (ess-r-version-time "R-1.9")
(defun ess-r-version-time (rver)
  "Return the time of the version of R named RVER.
The time is returned in seconds, to allow comparisons between
different versions of R.  If the version of R could not be found
from the output of the RVER program, -1 is returned."
  (let (ver-string 
	date 
	(time -1))
    (setq ver-string (shell-command-to-string 
		      (concat rver " --version")))
    (when (string-match "R version [^\n\(]+ (\\([0-9-]+\\))"  ver-string)
      (setq date (match-string 1 ver-string))
      (setq time (time-to-seconds
		  (apply #'encode-time 
			 (mapcar '(lambda (x) (if x x 0)) 
...
Create an alias for that version of R, so that M-x R-newest will run it."
  (let ( rnewest
	 (rtimes (mapcar 'ess-r-version-time rvers)))
    (setq rnewest (ess-which-newest rtimes rvers))
    (fset 'R-newest (intern rnewest))
    (message (concat "newest is " rnewest))))
    

(defun ess-which-newest (times rver)
  "Find the most recent version of RVER by seeing which of the corresponding
element of TIMES is newest (largest).  The time for each version of R is
computed by `ess-r-version-time'."
  (let 
      ( newest-time newest-ver)
    (setq newest-time (car times)
	  times    (cdr times))
    (setq newest-ver (car rver)
	  rver    (cdr rver))
    (while times
      (setq this-num (car times)
	    this-ver (car rver)
	    times     (cdr times)
	    rver     (cdr rver))
      (when (> this-num newest-time)
	(setq newest-ver this-ver
	      newest-time this-num)))
    newest-ver))
