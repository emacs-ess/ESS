...
;; FIXME: below we have a "copy-paste" version of this,
;; ess-rterm-versions-create  {which should really be merged into here}
...
instead call the defun `ess-find-newest-R'.")
...
*were* created.  Note that this really works from `ess-rterm-version-paths'!
...
This function was generated by `ess-r-versions-create'.\"
  (interactive \"P\")
  (let ((use-dialog-box nil);; Win-only: dialog box won't return a directory
        (inferior-R-program-name \"Rterm-X.Y\"))
    (R start-args)))
"
	   ))
	...
	;; Find which versions of R we want.  Remove the pathname, leaving just
	;; the name of the executable.
	(setq versions ess-rterm-version-paths)
	;;             ^^^^^^^^^^^^^^^^^^^^^^^ created in ./ess-site.el at start
	(ess-write-to-dribble-buffer
	 (format "(R): ess-rterm-versions-create making M-x defuns for \n %s"
		 (mapconcat 'identity versions "\n ")))
	...
	(while versions
	  (let* ((version (car versions))
	    ...
	    (setq r-versions-created
		  (cons version-root r-versions-created))
	    (setq versions (cdr versions))
	    (insert template)
	    (goto-char beg)
	    (while (search-forward "R-X.Y" nil t)
