...
;; Code for editing S source code. 
...
;;;; In this section:
;;;;
;;;; * The major mode essl-s-mode
;;;; * Commands for essl-s-mode
...
(if essl-S-mode-map
  ...
  (cond ((string-match "XEmacs\\|Lucid" emacs-version)
	 ;; Code for XEmacs
 	 (setq essl-s-mode-map (make-keymap))
	 (set-keymap-parent essl-s-mode-map text-mode-map)) ;; was comint?!?
	((not (string-match "XEmacs\\|Lucid" emacs-version))
	 ;; Code for GNU Emacs
	 (setq essl-s-mode-map (make-sparse-keymap))))

  ;; By popular demand:
  (define-key essl-s-mode-map "\C-m"        'newline-and-indent)
  (define-key essl-s-mode-map "\C-c\C-r"    'ess-eval-region)
  (define-key essl-s-mode-map "\C-c\M-r"    'ess-eval-region-and-go)
  (define-key essl-s-mode-map "\C-c\C-b"    'ess-eval-buffer)
  (define-key essl-s-mode-map "\C-c\M-b"    'ess-eval-buffer-and-go)
  (define-key essl-s-mode-map "\C-c\C-f"    'ess-eval-function)
  (define-key essl-s-mode-map "\C-c\M-f"    'ess-eval-function-and-go)
  (define-key essl-s-mode-map "\M-\C-x"     'ess-eval-function)
  (define-key essl-s-mode-map "\C-c\C-n"    'ess-eval-line-and-next-line)
  (define-key essl-s-mode-map "\C-c\C-j"    'ess-eval-line)
  (define-key essl-s-mode-map "\C-c\M-j"    'ess-eval-line-and-go)
  (define-key essl-s-mode-map "\M-\C-a"     'ess-beginning-of-function)
  (define-key essl-s-mode-map "\M-\C-e"     'ess-end-of-function)
  (define-key essl-s-mode-map "\C-c\C-y"    'ess-switch-to-S)
  (define-key essl-s-mode-map "\C-c\C-z"    'ess-switch-to-end-of-S)
  (define-key essl-s-mode-map "\C-c\C-l"    'ess-load-file)
  (define-key essl-s-mode-map "\C-c\C-v"    'ess-display-help-on-object)
  (define-key essl-s-mode-map "\C-c\C-d"    'ess-dump-object-into-edit-buffer)
;(define-key essl-s-mode-map "\C-c5\C-d"'ess-dump-object-into-edit-buffer-other-frame)
  (define-key essl-s-mode-map "\C-c\C-t"    'ess-execute-in-tb)
  (define-key essl-s-mode-map "\C-c\t"      'ess-complete-object-name)
  (define-key essl-s-mode-map "\M-\t"       'comint-replace-by-expanded-filename)
  (define-key essl-s-mode-map "\M-?"        'ess-list-object-completions)
  ;; wrong here (define-key essl-s-mode-map "\C-c\C-k" 'ess-request-a-process)
  (define-key essl-s-mode-map "\C-c\C-k"    'ess-force-buffer-current)
  (define-key essl-s-mode-map "\C-c`"       'ess-parse-errors) ; \C-x reserved!
  (define-key essl-s-mode-map "{"           'ess-electric-brace)
  (define-key essl-s-mode-map "}"           'ess-electric-brace)
  (define-key essl-s-mode-map "\e\C-h"      'ess-mark-function)
  (define-key essl-s-mode-map "\e\C-q"      'ess-indent-exp)
  (define-key essl-s-mode-map "\177"        'backward-delete-char-untabify)
  (define-key essl-s-mode-map "\t"          'ess-indent-command))
	...
	(setq nomessage (= (buffer-size) 0)))
	  ...
	  (message "Errors: Use %s to find error."
		   (substitute-command-keys
		    "\\<inferior-essl-s-mode-map>\\[ess-parse-errors]"))
